...
def main(filepath = "data/activations/offset10.pkl", bool_comput = 0, saving_filepath =  "data/RDM/offset10.pkl", num_trained_models = 5, session_id = 3):
    ...
    if bool_comput:
        ...
        plot_rdm(
            [neural_data_rdm_corr, embedding_rdm_corr],
            ["Neural input", "Output Layer"],
        ...
        for j in tqdm(range(num_trained_models)):
            embeddings_trained_multi_all = activations_multi[
                j * num_layers : (j + 1) * num_layers
            ] + [activations_multi[-output_embeddings_idxs[j]]]
            embeddings_trained_single_all = activations_single[
                j * num_layers : (j + 1) * num_layers
            ] + [activations_single[-output_embeddings_idxs[j]]]

            correlation_RDM_multi = []
            correlation_RDM_single = []

            for i in tqdm(range(len(embeddings_trained_multi_all))):
                rmd_corr_multi = pdist(
                    embeddings_trained_multi_all[i][:, idxs.flatten()].T,
                    metric="correlation",
                )
                correlation_RDM_multi.append(
                    1 - correlation(oracle_rdm, rmd_corr_multi)
                )

                rmd_corr_single = pdist(
                    embeddings_trained_single_all[i][:, idxs.flatten()].T,
                    metric="correlation",
                )
                correlation_RDM_single.append(
                    1 - correlation(oracle_rdm, rmd_corr_single)
                )
        ...
        embeddings_untrained_single_all = activations_UT[:num_layers] + [
            activations_UT[-2]
        ]
        embeddings_untrained_multi_all = activations_UT[num_layers:-2] + [
            activations_UT[-1]
        ]
        ...
        for i in tqdm(range(len(embeddings_untrained_multi_all))):
            ...
            rmd_corr_multi_UT = pdist(
                embeddings_untrained_multi_all[i][:, idxs.flatten()].T,
                metric="correlation",
            )
            correlation_RDM_multi.append(1 - correlation(oracle_rdm, rmd_corr_multi_UT))

            rmd_corr_single_UT = pdist(
                embeddings_untrained_single_all[i][:, idxs.flatten()].T,
                metric="correlation",
            )
            correlation_RDM_single.append(
                1 - correlation(oracle_rdm, rmd_corr_single_UT)
            )
    ...
    for i in range(2):
        sns.lineplot(
            x=np.arange(1, len(all_corrs["single"][i]) + 1),
            y=all_corrs["UT"][i],
            linestyle="--",
            marker="D",
            color=grey,
            alpha=0.5,
            label="Untrained" if i == 0 else "",
        )
    #  MULTI
    for i in range(num_trained_models):
        sns.lineplot(
            ...
            color=pastel_purple,
            alpha=0.5,
            label="Multi" if i == 0 else "",
        )
    #  SINGLE
    for i in range(num_trained_models):
        sns.lineplot(
            x=np.arange(1, len(all_corrs["single"][i]) + 1),
            y=all_corrs["single"][i],
            linestyle="-",
            marker="D",
            color=pastel_blue,
            alpha=0.5,
