...
def main(model_name, layer_type, session_id, filename, bool_plot_embeddings):
    ...
    if bool_plot_embeddings:
        ...
        for i in range(
            len(models["multi"])
        ):  # assumes that the same number of single sessions and multi sessions were trained

            embeddings_single.append(models["single"][i].transform(X))
            embeddings_multi.append(
                models["multi"][i].transform(X, session_id=session_id)
            )
        ...
        embeddings_untrained_single = models["UT"][0].transform(
            X
        )  # this assumes that single will always come first. This is true if they are named in the same convention but this should be changed by a reg expression of UT + single.
    ...
    def attach_hooks(
        model, name, bool_train=False, layer_type="conv"
    ):  # only attaches hooks on convolutional layers
        ...
        if layer_type == "conv":
            for i in range(len(model.net)):
                if isinstance(model.net[i], nn.Conv1d):
                    model.net[i].register_forward_hook(
                        get_activation(f"{name}{string_ut}_layer{num_layer}")
                    )
                    num_layer += 1
                    ...
                    for j in range(len(model.net[i].module)):
                        if isinstance(model.net[i].module[j], nn.Conv1d):
                            model.net[i].module[j].register_forward_hook(
                                get_activation(f"{name}{string_ut}_layer{i}")
                            )
                            num_layer += 1
    ...
    output_embeddings = {"UT": [], "single": [], "multi": []}
    # UNTRAINED
    attach_hooks(
        models["UT"][1].model_[session_id], "multi", False, layer_type=layer_type
    )
    ...
    output_embeddings["UT"].append(
        models["UT"][1].transform(train_data, session_id=session_id).T
    )
    ...
    for i, model in enumerate(models["multi"]):
        ...
        output_embeddings["multi"].append(
            model.transform(train_data, session_id=session_id).T
        )
