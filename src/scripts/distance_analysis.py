...
def main(
    ...
    if bool_comput:
        ...
        embeddings_untrained_single = activations_dict["act_UT"][:num_layers] + [
            activations_dict["act_UT"][-2]
        ]
        embeddings_untrained_multi = activations_dict["act_UT"][num_layers:-2] + [
            activations_dict["act_UT"][-1]
        ]

        distances_inter["UT"].append(
            compute_inter_bin_distances(embeddings_untrained_single, idxs, num_bins)
        )
        distances_inter["UT"].append(
            compute_inter_bin_distances(embeddings_untrained_multi, idxs, num_bins)
        )

        distances_intra["UT"].append(
            compute_intra_bin_distances(embeddings_untrained_single, idxs, num_bins)[0]
        )
        distances_intra["UT"].append(
            compute_intra_bin_distances(embeddings_untrained_multi, idxs, num_bins)[0]
        )

        distances_rep["UT"].append(
            compute_mean_repetition_distances(
                embeddings_untrained_single, idxs, num_bins, num_repetitions
            )
        )
        distances_rep["UT"].append(
            compute_mean_repetition_distances(
                embeddings_untrained_multi, idxs, num_bins, num_repetitions
            )
        )

        # Loop over the trained instances of each model and compute distances
        for j in range(num_trained_models):
            embeddings_trained_multi_all = activations_dict["act_multi"][
                j * num_layers : (j + 1) * num_layers
            ] + [activations_dict["act_multi"][-output_embeddings_idxs[j]]]
            embeddings_trained_single_all = activations_dict["act_single"][
                j * num_layers : (j + 1) * num_layers
            ] + [activations_dict["act_single"][-output_embeddings_idxs[j]]]

            # Compute inter-bin distances
            interbin_multi = compute_inter_bin_distances(
                embeddings_trained_multi_all, idxs, num_bins
            )
            interbin_single = compute_inter_bin_distances(
                embeddings_trained_single_all, idxs, num_bins
            )

            # Compute intra-bin distances
            intrabin_multi, _ = compute_intra_bin_distances(
                embeddings_trained_multi_all, idxs, num_bins
            )
            intrabin_single, _ = compute_intra_bin_distances(
                embeddings_trained_single_all, idxs, num_bins
            )

            # Compute interrep distances
            rep_multi = compute_mean_repetition_distances(
                embeddings_trained_multi_all, idxs, num_bins, num_repetitions
            )
            rep_single = compute_mean_repetition_distances(
                embeddings_trained_single_all, idxs, num_bins, num_repetitions
            )
    ...
    for key, dist in distances.items():
        ...
        plt.title(
            f"Evolution of Mean {key} Distance Across Layers (Multi vs Single); {filename}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=9,
        help="number of repetitions in your training data on which to do the repetition distance",
    )
