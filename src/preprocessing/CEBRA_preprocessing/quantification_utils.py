...
def compute_mean_repetition_distances(
    embeddings_list, idxs, num_classes, num_repetitions, metric="cosine"
):
...
def decoding_models(
    ...
    for i, model in enumerate(models_untrained):
        ...
        results_untrained[i] = decoding_frames(
    ...
    for i, model in enumerate(models_single):
        ...
        test = model.transform(data_test)
        results_single[i] = decoding_frames(
            embedding_train=train,
            label_train=label_train,
            embedding_test=test,
            label_test=label_test,
    ...
    for i, model in enumerate(models_multi):
        train = model.transform(data_train, session_id=3)
        test = model.transform(data_test, session_id=3)
        results_multi[i] = decoding_frames(
            embedding_train=train,
            label_train=label_train,
            embedding_test=test,
            label_test=label_test,
